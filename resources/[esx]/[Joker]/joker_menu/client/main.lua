...
function Text(text)
	...
	SetTextWrap(0.0, 1.0)
	SetTextCentre(false)
	SetTextDropshadow(163, 163, 163, 255)
	SetTextEdge(163, 163, 163, 0, 205)
	SetTextEntry("STRING")
...
function admin_godmode()
	...
	if godmode then
		SetEntityInvincible(plyPed, true)
		notification("GodMode", "Administration", "GodMode ~w~Activé")
	else
		SetEntityInvincible(plyPed, false)
		notification("GodMode", "Administration", "GodMode ~w~Désactivé")
	end
end
-- FIN GOD MODE
...
function admin_mode_fantome()
	...
	if invisible then
		SetEntityVisible(plyPed, false, false)
		notification("Invisible", "Administration", "Mode Fantôme ~w~Activé")
	else
		SetEntityVisible(plyPed, true, false)
		notification("Invisible", "Administration", "Mode Fantôme ~w~Désactivé")
	end
end
-- FIN INVISIBLE
...
function admin_vehicle_flip()
	...
	notification("Flip Véhicule", "Administration", "Retournement du véhicule ~w~effectué")
end
-- FIN flipVehicle
...
function admin_tp_marker()
	...
	if DoesBlipExist(WaypointHandle) then
		...
		notification("Téléportation", "Administration", "Téléportation ~w~Effectuée")
	else
		notification("Téléportation", "Administration", "Aucun ~w~Marqueur")
	end
end
-- FIN TP MARKER
...
Citizen.CreateThread(function()
	while true do
		...
		if showcoord then
			...
			Text("~w~X~s~: " .. playerPos.x .. " ~w~b~s~: " .. playerPos.y .. " ~g~Z~s~: " .. playerPos.z .. " ~w~Angle~s~: " .. playerHeading)
		end
