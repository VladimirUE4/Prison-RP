...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('esx_bucheronjob:getStockItems', function(items)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_bucheronjob:getStockItem', itemName, count)
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_bucheronjob:getPlayerInventory', function(inventory)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_bucheronjob:putStockItems', itemName, count)
						end
...
AddEventHandler('esx_bucheronjob:hasEnteredMarker', function(zone)
	if zone == 'MineFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack'  then
		CurrentAction     = 'raisin_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end

	if zone == 'MineFarm2' and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack'  then
		CurrentAction     = 'raisin_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end

	if zone == 'MineFarm3' and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack'  then
		CurrentAction     = 'raisin_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end

	if zone == 'MineFarm4' and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack'  then
		CurrentAction     = 'raisin_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end

	if zone == 'MineFarm5' and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack'  then
		CurrentAction     = 'raisin_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end

	if zone == 'MineFarm6' and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack'  then
		CurrentAction     = 'raisin_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end

	if zone == 'MineFarm7' and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack'  then
		CurrentAction     = 'raisin_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'TraitementFoundry' and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack'  then
		CurrentAction     = 'vine_traitement'
		CurrentActionMsg  = _U('press_traitement_stonew')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'TraitementStoneW' and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack'  then
		CurrentAction     = 'jus_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone = zone}
	end
		
	if zone == 'SellCopper' and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'SellIron' and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'SellGold' and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'SellDiamond' and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end
...
AddEventHandler('esx_bucheronjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'MineFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack' then
		TriggerServerEvent('esx_bucheronjob:stopHarvest')
	end 
	if (zone == 'MineFarm2') and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack' then
		TriggerServerEvent('esx_bucheronjob:stopHarvest')
	end
	...
	if (zone == 'TraitementFoundry' or zone == 'TraitementStoneW') and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack' then
		TriggerServerEvent('esx_bucheronjob:stopTransform')
	end
	if (zone == 'SellCopper') and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack' then
		TriggerServerEvent('esx_bucheronjob:stopSell')
	end
	if (zone == 'SellIron') and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack' then
		TriggerServerEvent('esx_bucheronjob:stopSell')
	end
    ...
    if PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack' then
		...
		for k,v in pairs(Config.Zones)do
			if v.Type == 1 then
				...
				SetBlipSprite (blip2, 0)
				SetBlipDisplay(blip2, 0)
				SetBlipScale  (blip2, 0)
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_bucheronjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_bucheronjob:hasExitedMarker', LastZone)
			end
		end
	end
end)
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'raisin_harvest' then
					TriggerServerEvent('esx_bucheronjob:startHarvest', CurrentActionData.zone)
					local playerPed = GetPlayerPed(-1)
					RequestAnimDict("amb")
				...
				if CurrentAction == 'jus_traitement' then
					TriggerServerEvent('esx_bucheronjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'vine_traitement' then
					TriggerServerEvent('esx_bucheronjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_bucheronjob:startSell', CurrentActionData.zone)
				end
