...
AddEventHandler('playerConnecting', function(user, set)
	...
	for k,v in ipairs(GetPlayerIdentifiers(source))do
		...
		if isBanned(v) then
			...
			set(GetConvar("es_admin_banreason", "Vous avez était ban."))
		...
		TriggerEvent('es:canGroupTarget', user.getGroup(), "admin", function(available)
			...
			if t == "group" then
				...
				if(GetPlayerName(USER) == nil)then
					...
					TriggerClientEvent('chatMessage', source, 'SERVER', {255, 0, 0}, "Player not found")
					...
					TriggerEvent("es:getAllGroups", function(groups)
						...
						if(groups[GROUP])then
							...
							TriggerEvent("es:setPlayerData", USER, "group", GROUP, function(response, success)
								...
								TriggerClientEvent('chatMessage', -1, "SERVER", {0, 0, 0}, "Group of ^2^*" .. GetPlayerName(tonumber(USER)) .. "^r^0 has been set to ^2^*" .. GROUP)

							end)

						else

							TriggerClientEvent('chatMessage', source, 'SERVER', {255, 0, 0}, "Group not found")
					...
					if(GROUP ~= nil and GROUP > -1)then
						...
						TriggerEvent("es:setPlayerData", USER, "permission_level", GROUP, function(response, success)
							...
							if(true)then
								...
								TriggerClientEvent('chatMessage', -1, "SERVER", {0, 0, 0}, "Permission level of ^2" .. GetPlayerName(tonumber(USER)) .. "^0 has been set to ^2 " .. tostring(GROUP))
						...
						TriggerClientEvent('chatMessage', source, 'SEVER', {255, 0, 0}, "Invalid integer entered")
...
TriggerEvent('es:addCommand', 'report', function(source, args, user)
	...
	TriggerClientEvent('chatMessage', source, "🔍 ┊ Ticket", {0, 0, 0}, " (^3" .. GetPlayerName(source) .." | "..source.."^0) " .. table.concat(args, " "))
		...
		for k,v in pairs(pl) do
			...
			TriggerEvent("es:getPlayerFromId", k, function(user)
				...
				if(user.getPermissions() > 0 and k ~= source)then
					...
					TriggerClientEvent('chatMessage', k, "🔍 ┊ Ticket", {0, 0, 0}, " (^3" .. GetPlayerName(source) .." | "..source.."^0) " .. table.concat(args, " "))
	...
	if args[1] then
		...
		if(GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('chatMessage', player, "🔍 ┊ Ticket", {0, 0, 0}, " ^3 Le Staff vous a TP ^2" .. GetPlayerName(source))

				TriggerClientEvent('chatMessage', source, "🔍 ┊ Ticket", {0, 0, 0}, "^3 Le joueur ^2" .. GetPlayerName(player) .. "^3 à était téléporter")
		...
		if(GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				if(target)then
					...
					TriggerClientEvent('chatMessage', player, "🔍 ┊ Ticket", {0, 0, 0}, " ^3Le staff à prit ton ticket! ^3" .. GetPlayerName(source))

					TriggerClientEvent('chatMessage', source, "🔍 ┊ Ticket", {0, 0, 0}, " ^3Tu as pris le ticket de ^3" .. GetPlayerName(player) .. "")
